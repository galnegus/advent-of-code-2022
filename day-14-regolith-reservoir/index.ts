...
const min: Coordinate = { x: Number.MAX_VALUE, y: Number.MAX_VALUE };
const max: Coordinate = { x: Number.MIN_VALUE, y: Number.MIN_VALUE };
const paths = input.split(/\r?\n/).map((line) =>
  line.split(" -> ").map((coordinateInput) => {
    const splitInput = coordinateInput.split(",").map(Number);
    const coordinate: Coordinate = { x: splitInput[0], y: splitInput[1] };
...
let unitsOfSand = 0;
while (true) {
  const sand: Coordinate = { x: 500, y: 0 };
